...
_ z = 1 // z is p_object, z._p points to heap object
        // refcount should be at object, not pointer
        // when p_object called destructor, refcount 
	// should be decremented
        // gc should have a vector of object*
        //
...
// The pool is a data structure of tracking pointers to objects and ref count.
// Note that the refcount is in this pool and not with the object.
// This is allow better performance optimization in case the object classes
// can be used without garbage collection. Or should I keep this in the
// p_object class?
// Should use a map of (object *, int).
class object;
class pool_entry
{
public:
  pool_entry(object * p, int r) : _p(p), _refcount(r) {}
  object * _p;
  int _refcount;
};
class pool
{
public:
  void add(object * p) { _p.push_back(pool_entry(p, 1)); }
  std::vector<pool_entry> _p;
};
...
void p_object::operator=(pyint i)   { _p = &i; } // FIX
void p_object::operator=(int i)   { _p = new pyint(i); } // need to release!
...
void p_object::operator=(pyfloat & i) { _p = &i; }
...
void p_object::operator=(char s[])   { _p = new pystr(s); } // need to release!
void p_object::operator=(pystr & s)   { _p = new pystr(s); } // need to release!
